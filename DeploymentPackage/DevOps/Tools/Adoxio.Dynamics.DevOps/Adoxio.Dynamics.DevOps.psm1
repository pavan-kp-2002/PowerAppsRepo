...
function GetSolutionPackagerFolder {
    if(Test-Path "$env:CRM_SDK_PATH\CoreTools\SolutionPackager.exe") { # CRM 9.x SDK
...
#   if(Test-Path "$env:CRM_SDK_PATH\PackageDeployer.exe") { # CRM 9.x SDK
...
function Export-CrmSolutions {
    ...
    process
    {
        ...
        Set-CrmConnectionTimeout -conn $CrmConnection -TimeoutInSeconds 1200
...
function Invoke-ImportCrmPackage {
    ...
    process
    {
        ...
        $securityProtocolVersion = [Net.ServicePointManager]::SecurityProtocol
         ...
         $LogFile = "$PackageFolder" +"\Microsoft.Xrm.Tooling.PackageDeployment-" + (Get-Date -Format yyyy-MM-dd) + ".log"
        ...
        try {
            ...
            $job = Start-Job -ScriptBlock {
                ...
                $crmConnection = Get-CrmConnection @crmConnectionParams

                Set-CrmConnectionTimeout -conn $crmConnection -TimeoutInSeconds 1200
                                
                Import-CrmPackage –CrmConnection $crmConnection –PackageDirectory (Split-Path $using:PackageFolder) –PackageName $using:PackageName -LogWriteDirectory (Split-Path $using:PackageFolder) -Verbose
                }
...
function NewCrmImportConfigXml {
    ...
    foreach($zipFile in $SolutionZipFiles) {
        ...
        $requiredimportmode = "requiredimportmode=""async"""
        $solutionsXml = $solutionsXml + "<configsolutionfile $solutionpackagefilenameattribute $overwriteunmanagedcustomizationsattribute $requiredimportmode />`n"    }
